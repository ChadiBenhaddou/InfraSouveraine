    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-950 via-blue-900 to-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <div class="flex justify-center mb-6">
                <img src="{{ asset('infra-white.png') }}" alt="InfraSouveraine" class="h-24 w-auto">
            </div>
            <h2 class="text-center text-3xl font-bold text-white">Connexion</h2>
            <p class="mt-2 text-center text-sm text-blue-200/70">
                Ou <a href="{{ route('register') }}" class="font-medium text-amber-400 hover:text-amber-300">créez un compte</a>
            </p>
        </div>
                ...
                <form class="space-y-6" method="POST" action="{{ route('login') }}">
                    ...
                    <div class="flex items-center">
                        <input id="remember" name="remember" type="checkbox"
                            class="h-4 w-4 rounded border-white/20 bg-white/10 text-amber-500 focus:ring-amber-500">
                        <label for="remember" class="ml-2 block text-sm text-blue-200">Se souvenir de moi</label>
                    </div>
                    <button type="submit"
